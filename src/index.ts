...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Scryfall');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'search_cards': {
      ...
      if (isError(res)) {
        const { error: status, message } = res as { error: number; message: string };
        // Scryfall answers a query that matches zero cards with a 404, not an
        // empty list. That is a no-result, not a defect — hand back the same
        // shape a zero-hit search would have, so it books as `empty`.
        if (status === 404) {
          return {
            total_cards: 0,
            has_more: false,
            cards: [],
            hint: `Scryfall matched no cards for "${query}". Check the search syntax at https://scryfall.com/docs/syntax, or broaden the query.`,
          };
        }
        // 400 is Scryfall rejecting the query syntax itself ("Your search
        // contains unclosed parentheses.") — the caller's query, not our bug.
        if (status === 400) {
          return {
            error: 'user_error',
            message: `Scryfall rejected the query "${query}": ${message} Fix the search syntax (see https://scryfall.com/docs/syntax) and retry.`,
          };
        }
        return res;
      }
